...
def parse_args():
    ...
    p.add_argument("--div_csv",    default="total_hunk_divergence_results.csv",
...
def load_total_divergence(path: str):
    d = {}
    with open(path, newline="", encoding="utf-8") as f:
        rdr = csv.DictReader(f)
        for col in ("bug_id", "divergence"):
...
def build_feature_matrix(div_map, passed_map, prox_map):
    rows = []
    for bug, div in div_map.items():          # iterate over all total-div entries
        prox   = prox_map.get(bug, "UNKNOWN")
        label  = int(any(bug in s for s in passed_map.values()))
        rows.append({
            "bug":         bug,
            "divergence":  div,
...
def main():
    ...
    X_num    = df[["divergence"]].values          # 1 numeric column
    X        = np.hstack([X_num, prox_ohe.values])
    y        = df["label"].values
    feat_names = ["divergence"] + [f"prox_{c}" for c in PROX_CLASSES]
